...
class TableTest extends WordSpecLike with Matchers with BeforeAndAfterAll {
  ...
  "Table" should {
    ...
    "can be created from diagonals" in {
      Table.fromDiagonals(List(List(1)), 1, 1).values should === (List(List(1)))
      Table.fromDiagonals(List(List(1),List(3,2), List(4)), 2, 2).values should === (List(List(1,2), List(3,4)))
//      Table.fromDiagonals(
//        List(
//          List(1),
//          List(5,2),
